...
export async function GET(req: Request) {
  try {
    ...
    if (!token) {
      return apiJson({ ok: false, message: "personels token tanimli degil." }, 500)
    }
...
export async function POST(req: Request) {
  try {
    ...
    if (!name || !phone) {
      return apiJson({ ok: false, message: "Ad soyad ve telefon zorunlu." }, 400)
    }
