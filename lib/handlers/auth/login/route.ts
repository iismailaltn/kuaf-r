...
export async function POST(req: Request) {
  try {
    ...
    if (!username || !password) {
      return apiJson({ ok: false, message: "Kullanici adi ve sifre zorunlu." }, 400)
    }
    ...
    if (!row) {
      return apiJson({ ok: false, message: "Giris basarisiz." }, 401)
    }
    ...
    if (!passwordHash) {
      return apiJson({ ok: false, message: "Kullanici sifresi bulunamadi." }, 500)
    }
