...
export async function GET(req: Request) {
  try {
    ...
    if (!operationsToken) {
      return NextResponse.json({ ok: false, message: "session_operations token tanimli degil." }, { status: 500 })
    }
    if (!itemsToken) {
      return NextResponse.json({ ok: false, message: "session_operation_items token tanimli degil." }, { status: 500 })
    }
...
export async function POST(req: Request) {
  try {
    ...
    if (!customerNameRaw || !customerSurnameRaw) {
      return NextResponse.json({ ok: false, message: "Musteri adi ve soyadi zorunlu." }, { status: 400 })
    }
    if (!workspaceNameRaw) {
      return NextResponse.json({ ok: false, message: "Calisma alani adi zorunlu." }, { status: 400 })
    }
    if (!staffIdRaw || !staffNameRaw) {
      return NextResponse.json({ ok: false, message: "Personel bilgisi zorunlu." }, { status: 400 })
    }
    ...
    if (serviceItems.length === 0) {
      return NextResponse.json({ ok: false, message: "En az bir hizmet zorunlu." }, { status: 400 })
    }

    for (const item of serviceItems) {
      if (!Number.isFinite(item.price) || item.price < 0) {
        return NextResponse.json({ ok: false, message: "Gecersiz hizmet fiyati." }, { status: 400 })
      }
    }
    ...
    if (!sessionOperationId) {
      return NextResponse.json({ ok: false, message: "Islem kaydi olusturuldu ancak satirlar baglanamadi." }, { status: 502 })
    }
